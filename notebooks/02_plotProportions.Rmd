...
Below, we plot a PCA for the proportions. Note, the major souce of variance is the time, since we will enevitably have more nascent protein at 24h than 3h or 6h. The effect of GC7 is 1-2 orders of magnitude less than the difference between the time points.

```{r, fig.height=5, fig.width=8}
pca <- prot_proportions %>%
  filterNA() %>%
  exprs() %>%
  t() %>%
  prcomp(scale=TRUE, centre=TRUE)
...
p <- pca$x %>%
  ...
  separate(sample, into=c('Time', 'Treatment', 'replicate')) %>%
  mutate(Time=factor(Time, levels=c(3,6,24))) %>%
  mutate(Treatment=recode(Treatment, 'untrt'='Control')) %>%
  ggplot(aes(PC1, PC2, fill=Time, shape=Treatment)) +
  geom_point(size=5) +
  scale_shape_manual(values=c(21,25)) +
  scale_fill_manual(values=get_cat_palette(3),
                    guide = guide_legend(override.aes = list(size = 6, shape = 21)),
                    name='Time (h)') +
  xlab(sprintf('PC1 (%s%%)', 100*round(pov[1], 2))) +
  ylab(sprintf('PC2 (%s%%)', 100*round(pov[2], 2))) +
  theme(aspect.ratio=1)

print(p)

ggsave(here('figures/plots/pca_proportions.pdf'))
ggsave(here('figures/plots/pca_proportions.png'))
